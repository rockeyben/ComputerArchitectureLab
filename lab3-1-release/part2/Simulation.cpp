...
{	
	...
	// co-lab with lab3.1
	// add cache to instruction Mem and data Mem

	//cache settings
	CacheConfig l1_config, l2_config, llc_config;
	StorageStats s;
	StorageLatency l1_lat, l2_lat, llc_lat, m_lat;
	...
	l2_lat.hit_latency = 8;

	llc_config.id = 3;
	llc_config.size = 8 * 1024;
	llc_config.block_size = 64;
	llc_config.associativity = 8;
	llc_config.write_through = 0;
	llc_config.set_num = llc_config.size * 1024 / 64 / 8;
	llc_lat.hit_latency = 20;

	m_lat.hit_latency = 100;
	s.access_time = 0;
	s.access_counter = 0;
	...
	l1.SetLatency(l1_lat);
	l2.SetLatency(l2_lat);
	llc.SetLatency(llc_lat);
	m.SetLatency(m_lat);
	...
	while(F_REG.PC!=end)
	{
		...
		else
		{
			...
			if(data_time > 1)
			{
				...
				D_REG.stall = 1;
				F_REG.stall = 1;
				o_data_time = data_time;
				data_time --;
			}

			if(data_time == 1 && o_data_time == 2)
			{
				M_REG.stall = 0;
				E_REG.stall = 0;
				D_REG.stall = 0;
			...
			if(inst_time > 1)
			{
				o_inst_time = inst_time;
				F_REG.stall = 1;
				inst_time --;
			}
			
			if(inst_time == 1 && o_inst_time == 2)
			{
				F_REG.stall = 0;
			}
		}
		...
		if(F_REG.stall == 1 && D_REG.stall == 0)
		{
			D_REG.clearReg();
		}

		if(M_REG.stall == 1 && W_REG.stall == 0)
		{
			W_REG.clearReg();
		}

		if(E_REG.stall == 1 && M_REG.stall == 0)
		{
			M_REG.clearReg();
		}
...
void IF(int & inst_time)
{
	...
	if(F_REG.stall == 0)
	{
		...
		char content[64];
		int hit;
		l1.HandleRequest(F_REG.PC<<2, 4, 1, content, hit, inst_time);
		F_REG.IR = fetch_ui(content);
...
void MEM(int & data_time)
{
	...
	if(M_REG.stall == 0)
	{
		...
		int hit;
		ll waddr = M_REG.ALUOUT;
		ll wdata = M_REG.RS2;
		wb_sel.set_input(M_REG.ALUOUT, 0);
		//cout<<hex<<"waddr: "<<waddr<<endl;

		if(M_REG.ctrl_M_MemWrite>0)
		{
			unsigned ll one = 1;
			unsigned int tmpdata[2]={0};
			char * content;
			switch(M_REG.ctrl_M_MemWrite)
			{
				...
				case 1:
					wdata = wdata & ((one << 8) - 1);
					tmpdata[0] = wdata;
					content = (char*)tmpdata;
					l1.HandleRequest(waddr, 1, 0, content, hit, data_time);
				...
				case 2:
					...
					tmpdata[0] = wdata;
					content = (char*)tmpdata;
					l1.HandleRequest(waddr, 2, 0, content, hit, data_time);
					//memory[waddr] = (unsigned int)(wdata);
					break;
				case 3:
					...
					tmpdata[0] = wdata;
					content = (char*)tmpdata;
					l1.HandleRequest(waddr, 4, 0, content, hit, data_time);
					break;
				case 4:
				{
					...
					tmpdata[1] = dh;
					content = (char*)tmpdata;
					l1.HandleRequest(waddr, 8, 0, content, hit, data_time);
					//memory[waddr] = dl;
					//memory[waddr + 1] = dh;
				}
					break;
		...
		if(M_REG.ctrl_M_MemRead>0)
		{
			...
			unsigned int tmpdata[2]={0};
			char * content = (char*)tmpdata;
			switch(M_REG.ctrl_M_MemRead)
			{
				case 1:
					//rdataL = memory[waddr];
					l1.HandleRequest(waddr, 1, 1, content, hit, data_time);
					rdataL = fetch_ui(content);
					//rdataL = rdataL & ((1 << 8) - 1);
					rdata = ext_signed_ll(rdataL, 7);
					break;
				case 2:
					//rdataL = memory[waddr];
					l1.HandleRequest(waddr, 2, 1, content, hit, data_time);
					rdataL = fetch_ui(content);
				...
				case 3:
					//rdataL = memory[waddr];
					l1.HandleRequest(waddr, 4, 1, content, hit, data_time);
					rdataL = fetch_ui(content);
				...
				case 4:
				{
					...
					l1.HandleRequest(waddr, 8, 1, content, hit, data_time);
					rdataL = fetch_ui(content);
					rdataH = fetch_ui(content + 4);
