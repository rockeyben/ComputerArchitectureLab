...
void MEM()
{
	...
	wb_sel.set_input(ALUOUT, 0);

	if(ctrl_M_MemWrite>0)
	{
		unsigned ll one = 1;
		switch(ctrl_M_MemWrite)
		{
			case 1:
				wdata = wdata & ((one << 8) - 1);
...
void WB()
{
	...
	if(ctrl_W_WriteReg==1)
	{
		//printf("WBData:%lx\n", W_REG.WBData);
		reg[G_rd] = (unsigned ll)(WBData);
		ctrl_W_WriteReg = 0;
	}
}
